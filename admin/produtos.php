<?php
  ...
  if(isset($_POST['busca'])){
    ...
    $sistema=$pdo->prepare("SELECT * FROM {$tabela}
                                     WHERE (titulo LIKE :busca OR resumo LIKE :busca OR descritivo LIKE :busca)
                                     ORDER BY dataUpdate DESC LIMIT 50");
    $sistema->bindParam(':busca', $busca, PDO::PARAM_STR);
  }else{
    $busca = "";
    $sistema=$pdo->prepare("SELECT * FROM {$tabela} ORDER BY dataUpdate DESC LIMIT 50");
  }
            ...
            <div class="list-group-item d-flex align-items-center"  data-aos="fade-up">
              ...
              <div class="me-auto p-3">
                ...
                <p class="busca-texto fs-sm text-muted mb-3">
                  <i class="fas fa-angle-right"></i> <?= $prodCat[$sistema->tipo1]; ?><br>
                  <i class="fas fa-angle-right"></i> <?= $prodCat[$sistema->tipo2]; ?><br>
                  <i class="fas fa-angle-right"></i> <?= $prodCat[$sistema->tipo3]; ?><br>
                </p>
