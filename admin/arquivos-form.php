<?php
  ...
  require_once('arquivos-banco.php');
      ...
      <div class="container">
        <div class="row bg-white rounded">
            ...
            <div class="col-12 col-md-9">
              ...
              <div class="p-3 mb-0">
                <label class="text-muted mb-3" for="tipo"><i class="fas fa-angle-right"></i> Categoria do arquivo: <i class="fas fa-question-circle" data-bs-toggle="tooltip" data-placement="top" title="" data-bs-original-title="Selecione a categoria aonde será listado o arquivo no site" aria-label="visualizar"></i></label>
                <select id="tipo" name="tipo" class="form-select form-select-sm" data-choices>
                  <option disabled>Escolha a categoria do Arquivo</option>
                  <?php foreach ($arquivosCat as $arquivosCod => $arquivosTitulo) { if($arquivosCod != 0){ ?>
                  <option value="<?= $arquivosCod ?>" <?php if($arquivosCod == $campo_tipo){echo "selected";} ?>><?= $arquivosTitulo ?></option>
                  <?php }} ?>
                </select>
              </div>
